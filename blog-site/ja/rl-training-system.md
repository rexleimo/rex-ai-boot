...
date: 2026-03-27
description: shell・ブラウザ・オーケストレーターのタスクを横断して共有 student を学習するマルチ環境 RL を、1 つの統一コントロールプレーンで実装した方法。
---
...
環境アダプタは「実行に依存する部分」だけを持ちます。タスクサンプリング、エピソード実行、証拠収集、環境固有の検証入力などです。RL ロジックは `RL Core` に漏らしません。
