...
date: 2026-03-27
description: shell・ブラウザ・オ케ストレーターのタスクを横断して共有 student を学習するマルチ環境 RL を、1 つの統一コントロールプレーンで実装した方法。
---

# AIOS RL Training System: 合成バグ修正からマルチ環境学習へ

`aios orchestrate` のライブ実行を出して以来、私たちはその下にあるもっと深い仕組みを作ってきました。shell、ブラウザ、オケストレーターのタスクを横断して **1 つの student ポリシー**を継続的に改善する **マルチ環境強化学習（RL）システム**です。すべての環境は、統一された学習コントロールプレーンを共有します。
